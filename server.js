...
app.post('/post', (req, res) => {
    ...
    db.run('INSERT INTO messages (username, content, planet) VALUES (?, ?, ?)', 
           [username, content, planet || 'Unknown'], (err) => {
        if (err) return res.status(500).send('Error posting message');
...
app.get('/messages', (req, res) => {
    db.all('SELECT id, username, content, planet, timestamp FROM messages ORDER BY id DESC', [], (err, rows) => {
...
app.get('/spice-transactions', (req, res) => {
    db.all('SELECT * FROM spice_transactions ORDER BY transaction_date DESC', [], (err, rows) => {
        if (err) return res.status(500).json({ error: 'DB error' });
        res.json(rows);
    });
});
...
app.post('/register', (req, res) => {
    ...
    db.run('INSERT INTO users (username, password, house) VALUES (?, ?, ?)', 
